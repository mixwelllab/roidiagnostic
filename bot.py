...
— предложить простую процессную схему, показывающую ГДЕ проблема
...
— Не проектируй решения (никаких “внедрить”, “сделать систему”, “создать бота/CRM/ИИ”, “автоматизировать”).
...
Важно:
— максимум 4–5 блоков
— если блоков больше — ты усложнил
— мы НЕ описываем бизнес целиком
— мы показываем где система ломается

Разрыв должен быть системным:
нет правила / нет фиксации / нет контроля

Если данных недостаточно — делай гипотезу и помечай:
"Гипотеза"

━━━━━━━━━━━━━━━━━━━
ПРИМЕРЫ ПРОЦЕССНОЙ СХЕМЫ
━━━━━━━━━━━━━━━━━━━

Пример 1 — No-show (клиент не приезжает)

Запись клиента → Нет подтверждения записи → Клиент не приезжает → Пустой рабочий слот → Потеря выручки

Разрыв: нет подтверждения записи

Пример 2 — Скидки без правил

Расчёт стоимости → Менеджер даёт скидку → Маржа падает → Теряется прибыль

Разрыв: нет лимитов скидок

━━━━━━━━━━━━━━━━━━━
ПРОВЕРКА ПРИМЕНИМОСТИ ЗОНЫ
━━━━━━━━━━━━━━━━━━━

Вероятно применима:
— есть признаки проблемы
— она повторяется
— есть логическая связь с деньгами

Сомнительно:
— данных мало
— связь с деньгами неясна

Не применима:
— описание не соответствует зоне
...
Процессная схема (Событие → Разрыв → Последствие → Деньги):

… → … → … → …

ROI-зона (как именно здесь теряются деньги): …
...
Вероятный разрыв:
(правило / фиксация / контроль) + объяснение
...
— пункты 3 и 4 не одинаковые  
— процессная схема не длиннее 4–5 блоков  
— нет предложений решений  
— финансовый эффект выражен через деньги/маржу/выручку/себестоимость/потери
"""
